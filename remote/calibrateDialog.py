...
class CalibrateDialog(QtGui.QDialog):
    ...
    def onTry(self):
        """Close the dialog"""
        self.parent.bluetooth.send(Message(channel = "calibrateForward", value = "%d:%d:%d" % (self.time_slider.value(), self.speedL_slider.value(), self.speedR_slider.value())))
        self.parent.bluetooth.send(Message(channel = "calibrateForward", value = "test"))
